	...
	while(Serial.available() && !failed_message){
		...
		switch(this->listen_state){
			...
			case STATE_PRE_DATA:
				...
				if (Serial.available() >= sizeof(this->inc_length_bytes)){

					for (int i = 0; i < sizeof(inc_length_bytes); i++) { this->inc_length_bytes[i] = Serial.read(); }
			...
			case STATE_DATA:
				...
				// Used data length calc'd in STATE_PRE_DATA to check for appropriate amount of bytes + crc bytes
				if (Serial.available() >= this->inc_data_length + sizeof(this->inc_crc_bytes)){
					...
					uint32_t true_crc = this->generate_crc32(this->inc_data, this->inc_data_length);
...
bool UARTDriver::is_acquired(){
  return this->acquired;
}

void UARTDriver::set_acquired(bool acquired){
  this->acquired = acquired;
}

  uint8_t* UARTDriver::get_data(){
	  return this->inc_data;
  }
  int UARTDriver::get_data_length(){
	  return this->inc_data_length;
  }